...
def read_plate_fast_ocr(plate_img: np.ndarray, recognizer) -> tuple:
...
def read_plate_multiple_methods(plate_img: np.ndarray, recognizer) -> tuple:
    """
    ...
    if config.USE_FAST_OCR:
        text_lpr, conf_lpr = read_plate_fast_ocr(plate_img, recognizer)
...
def process_plate_ocr(
    plate_img: np.ndarray, track_id: int, frame_count: int, plate_index: int, recognizer
) -> tuple:

    if config.SAVE_PLATE_IMAGES:
        save_plate_image(plate_img, track_id, frame_count, plate_index)

    plate_text, ocr_conf, method = read_plate_multiple_methods(plate_img, recognizer)
...
def find_best_plate_reading(plates: list, track_id: int, frame_count: int, recognizer) -> tuple:
    best_result = None
    best_confidence = 0.0
    ...
    for i, plate_data in enumerate(plates):
        ...
        plate_text, ocr_conf, method = process_plate_ocr(
            plate_img, track_id, frame_count, i, recognizer
        )
